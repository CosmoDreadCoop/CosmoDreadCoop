...
using SteamScreenshot = Steamworks.Data.Screenshot;
...
namespace Steamworks
{
	...
	public class SteamScreenshots : SteamClientClass<SteamScreenshots>
	{
		...
		internal static void InstallEvents()
		{
			...
			Dispatch.Install<ScreenshotReady_t>( x =>
			{
				if ( x.Result != Result.OK )
					...
					OnScreenshotReady?.Invoke( new SteamScreenshot { Value = x.Local } );
			} );
		}
		...
		/// </summary>
		public static event Action<SteamScreenshot> OnScreenshotReady;
		...
		/// </summary>
		public unsafe static SteamScreenshot? WriteScreenshot( byte[] data, int width, int height )
		{
			fixed ( byte* ptr = data )
			{
				...
				return new SteamScreenshot { Value = handle };
			}
		}
		...
		/// </summary>
		public unsafe static Data.Screenshot? AddScreenshot( string filename, string thumbnail, int width, int height )
		{
